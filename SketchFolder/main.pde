// MainSketch.pde
// Main UI sketch: Frequency + Step + Mode boxes
// All working with the RC-28
// Loads config.json and frequencies.json
...
void draw() {
  ...
  // Draw the Frequency + Step + Mode UI
  civWin.drawUI();
}
...
void keyPressed() {
  ...
  if (civWin != null) {
    ...
    // Optional keyboard test for knob modes:
    if (key == 'q' || key == 'Q') civWin.handleKnobCCW();
    if (key == 'w' || key == 'W') civWin.handleKnobCW();
    if (key == 'a' || key == 'A') civWin.handleTXShortPress();
    if (key == 'z' || key == 'Z') civWin.handleTXLongPress();
  }
}
