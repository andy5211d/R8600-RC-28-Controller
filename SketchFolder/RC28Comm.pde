// RC28Comm.pde
// Encapsulates HID communication and decoding for Icom RC-28
...
class RC28Decoder {
  ...
  void decode(byte[] buffer, int len) {
    ...
    // Optional: JSON-driven frequency presets also applied via CIVComms
  }
  ...
  void handleButton(String name, boolean pressed) {
    ...
    if ("F1".equals(name)) {
      ...
      if (!pressed && txDown != -1) {
        ...
        boolean longPress = duration >= longPressThreshold;
        logs.add("[Tx] " + (longPress ? "Long" : "Short") + " press (" + duration + "ms)");

        if (longPress) {
          civ.handleTXLongPress();
        } else {
          civ.handleTXShortPress();
        }
